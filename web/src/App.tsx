...
function App() {
  ...
  if (!authenticated) {
    if (showAuth) return <AuthPanel onBack={() => setShowAuth(false)} onAuthenticated={() => { setAuthenticated(true); setShowAuth(false); setShowQuickCheck(false); setShowIntro(false) }} />
    if (showQuickCheck) return <main className="shell"><header><img className="name-logo" src={nameLogo} alt="PETAdvice" /><button className="cancel-fresh" onClick={() => setShowQuickCheck(false)}>Back</button></header><Triage pet={null} onLogin={() => setShowAuth(true)} /><Emergency /></main>
    return <PublicLanding onQuickCheck={() => setShowQuickCheck(true)} onLogin={() => setShowAuth(true)} />
  }
...
function PublicLanding({ onQuickCheck, onLogin }: { onQuickCheck: () => void; onLogin: () => void }) {
  return <main className="shell"><header><img className="main-logo" src={fullLogo} alt="PETAdvice dog and cat logo" /><button className="start-fresh" onClick={onLogin}>Log in</button></header><section className="intro"><p className="eyebrow">Friendly guidance for everyday pet worries</p><h1>A clearer next step for your pet.</h1><p className="intro-lead">When something feels different, PETAdvice helps you work out what to look for, how urgently to act, and what to discuss with your vet.</p><div className="intro-points"><article><span>01</span><div><h2>Make sense of symptoms</h2><p>Find straightforward information about common concerns, without confusing medical language.</p></div></article><article><span>02</span><div><h2>Know how urgent it is</h2><p>Answer a few focused questions to decide whether to monitor at home or contact a vet.</p></div></article><article><span>03</span><div><h2>Keep care on track</h2><p>Save pet profiles and keep your checks together when you create an account.</p></div></article></div><p className="intro-boundary">Start a quick check without an account. Create an account when you want to save your pets and history.</p><div className="intro-actions"><button className="primary intro-start" onClick={onQuickCheck}>Start a quick check <span>→</span></button><button className="secondary intro-secondary" onClick={onLogin}>Save my pet profiles</button></div></section><Emergency /></main>
}

function AuthPanel({ onBack, onAuthenticated }: { onBack: () => void; onAuthenticated: () => void }) {
...
function Triage({ pet, onLogin }: { pet: Pet | null; onLogin?: () => void }) {
  ...
  async function begin() { const started = await api.startTriage(pet?.id ?? null, symptom, pet?.species ?? species); setSession(started.session_id); if (started.flow_complete) setResult(await api.result(started.session_id)); else setQuestion(started.next_question) }
  ...
  return <section className="panel"><p className="eyebrow">Triage, without the guesswork</p><h2>What have you noticed?</h2><p className="muted">Choose the closest match. This check gives urgency guidance, not a diagnosis.</p>{!pet && <label className="guest-species">Who is this check for?<select value={species} onChange={(event) => setSpecies(event.target.value as 'dog' | 'cat')}><option value="dog">Dog</option><option value="cat">Cat</option></select></label>}<div className="symptoms">{symptoms.map((item) => <button className={symptom === item.value ? 'selected' : ''} onClick={() => setSymptom(item.value)} key={item.value}>{item.label}<span>＋</span></button>)}</div><button className="primary wide" disabled={!symptom} onClick={begin}>Continue <span>→</span></button></section>
}
